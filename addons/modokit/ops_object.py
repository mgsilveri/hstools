...
class OBJECT_OT_modo_click_select(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        if state._active_transform_mode in ('TRANSLATE', 'ROTATE', 'RESIZE'):
            # If the click lands on a move or scale gizmo handle, pass through
            # so view3d.modo_move_gizmo_drag / scale_gizmo_drag can start the drag.
            if state._active_transform_mode == 'TRANSLATE':
                from .transform_3d import _mg_hit_test
                if _mg_hit_test(self.mouse_x, self.mouse_y):
                    return {'PASS_THROUGH'}
            elif state._active_transform_mode == 'RESIZE':
                from .transform_3d import _sg_hit_test
                if _sg_hit_test(self.mouse_x, self.mouse_y):
                    return {'PASS_THROUGH'}
            from bpy_extras import view3d_utils
