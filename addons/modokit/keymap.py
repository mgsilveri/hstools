...
def register_keymaps():
    ...
    if kc:
        ...
        if prefs.enable_component_mode:
            ...
            # S = cycle falloff shape  (poll gates on fp.enabled, so S still
            # works as scale when falloff is off)
            for target_km in (km_nav, km_obj, km):
                kmi = target_km.keymap_items.new(
                    'view3d.modo_falloff_cycle_shape',
                    type='S', value='PRESS', head=True,
                )
                state.addon_keymaps.append((target_km, kmi))

            # D = reverse falloff (swap Start ↔ End)
            for target_km in (km_nav, km_obj, km):
                kmi = target_km.keymap_items.new(
                    'view3d.modo_falloff_reverse',
                    type='D', value='PRESS', head=True,
                )
                state.addon_keymaps.append((target_km, kmi))
